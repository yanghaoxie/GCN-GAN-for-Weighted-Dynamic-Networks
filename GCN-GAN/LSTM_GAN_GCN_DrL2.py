...
def gen_net(noise_input_phs, gcn_fact_phs):
    ...
    for i in range(window_size+1):
        ...
        #gcn_output = tf.nn.dropout(tf.sigmoid(tf.matmul(gcn_conv, gcn_wei)), keep_prob=1-dropout_rate)
        # +----------+
    ...
    LSTM_cells = rnn.MultiRNNCell([rnn.BasicLSTMCell(node_num*gen_hid_num0)])
    #LSTM_cells = rnn.DropoutWrapper(cell=LSTM_cells, input_keep_prob=1.0, output_keep_prob=1-dropout_rate)
    ...
    #gen_output = tf.nn.dropout(tf.nn.sigmoid(tf.matmul(LSTM_outputs[-1], gen_output_wei) + gen_output_bias), keep_prob=1-dropout_rate)
...
def disc_net(disc_input):
    ...
    #disc_h1 = tf.nn.dropout(tf.nn.sigmoid(tf.matmul(disc_input, disc_wei1) + disc_bias1), keep_prob=1-dropout_rate)
    # Hidden layer #1 -> Output layer
    disc_logit = tf.matmul(disc_h1, disc_wei2) + disc_bias2
    disc_output = tf.nn.sigmoid(disc_logit)
    #disc_output = tf.nn.dropout(tf.nn.sigmoid(disc_logit), keep_prob=1-dropout_rate)
...
# +--------------------+
# Lead the dynamic network 加载动态网络
adjs = [] # Sequence to store the adjacency matrices of all the time slices 保存所有时间片邻接矩阵的序列
for time_index in range(time_num):
    adjs.append(read_data(name_pre, time_index, node_num, max_thres))
max_elem = np.max(adjs) # Maximum edge weight 最大边权值
...
for var in gcn_weis:
    pre_gen_loss += decay_rate*tf.nn.l2_loss(var)
for var in LSTM_params:
    pre_gen_loss += decay_rate*tf.nn.l2_loss(var)
pre_gen_loss += decay_rate*tf.nn.l2_loss(gen_output_wei)
pre_gen_loss += decay_rate*tf.nn.l2_loss(gen_output_bias)
...
for t in range(window_size, time_num-3):
    ...
    for k in range(t-window_size, t+1):
        adj = adjs[k]/max_elem # Normalize the adjacency matrix 归一化邻接矩阵
        gcn_fact = get_gcn_fact(adj)
        gcn_facts.append(gcn_fact)
    # +--------------------+
    # Construct the ground-truth vector 构建标准答案向量
    gnd = np.reshape(adjs[t+1], (1, node_num*node_num))
    gnd /= max_elem
    ...
    adj_est = np.reshape(output[0]*max_elem, (node_num, node_num))
    ...
    #exit()
    f = open("+UCSB-LSTM_GAN_GCN(DrL2)-error.txt", 'a+')
    #f = open("+KAIST-LSTM_GAN_GCN(DrL2)_error.txt", 'a+')
    f.write('%d %f' % (t + 2, error))
    f.write('\n')
    f.close()

    f = open("+UCSB-LSTM_GAN_GCN(DrL2)-KL.txt", 'a+')
    #f = open("+KAIST-LSTM_GAN_GCN(DrL2)_KL.txt", 'a+')
    f.write('%d %f' % (t + 2, edge_wei_KL))
    f.write('\n')
    f.close()

    f = open("+UCSB-LSTM_GAN_GCN(DrL2)-mis.txt", 'a+')
    #f = open("+KAIST-LSTM_GAN_GCN(DrL2)-mis.txt", 'a+')
